...
class SerializerIntegrationTest(APITestCase, QuickTestDataMixin):
    ...
    def test_metadata_table_crud_serialization(self):
        ...
        # Simulate save
        table = MetadataTable.objects.create(creator=self.user, **serializer.validated_data)
        ...
        self.assertEqual(retrieve_data["creator_username"], self.user.username)
    ...
    def test_complex_data_serialization_performance(self):
        ...
        table = MetadataTableFactory.create_with_columns(user=self.user, column_count=20, sample_count=50)

        # Add multiple pools
        for i in range(5):
            SamplePoolFactory.create_pool(
                metadata_table=table, pool_name=f"Pool {i}", pooled_only_samples=list(range(i * 5, (i + 1) * 5))
            )
        ...
        self.assertEqual(data["column_count"], 20)
