...
class SDRFParsingIntegrationTest(TestCase, QuickTestDataMixin):
    ...
    def read_fixture_content(self, filename):
    ...
    def test_parse_pdc000126_fixture(self):
        ...
        content = self.read_fixture_content("PDC000126.sdrf.tsv")
        if not content:
            return
    ...
    def test_parse_pxd002137_fixture(self):
        ...
        content = self.read_fixture_content("PXD002137.sdrf.tsv")
        if not content:
            return
    ...
    def test_parse_pxd019185_fixture(self):
        ...
        content = self.read_fixture_content("PXD019185_PXD018883.sdrf.tsv")
        if not content:
            return
...
class SDRFExportIntegrationTest(TestCase, QuickTestDataMixin):
    ...
    def test_export_basic_sdrf(self):
        ...
        columns_data = [
            ...
            ("characteristics", "organism", 1, "homo sapiens", True),
            ("characteristics", "organism part", 2, "liver", True),
            ("characteristics", "disease", 3, "normal", True),
            ("assay name", "", 4, "run 1", True),
            ("technology type", "comment", 5, "proteomic profiling by mass spectrometry", False),
        ]
        ...
        self.assertEqual(header[0], "source name[]")  # No type for source name
        self.assertEqual(header[1], "organism[characteristics]")
        self.assertEqual(header[2], "organism part[characteristics]")
        self.assertEqual(header[5], "technology type[comment]")
    ...
    def test_export_with_pools(self):
        ...
        header = result_data[0]
        self.assertIn("pooled sample[characteristics]", header)
    ...
    def test_export_to_tsv_format(self):
        ...
        columns_data = [
            ...
            ("characteristics", "organism part", "endometrium"),
            ("characteristics", "disease", "cervical endometrioid adenocarcinoma"),
            ("comment", "instrument", "NT=Orbitrap Fusion Lumos;AC=MS:1002732"),
            ("assay name", "", "run"),
            ("technology type", "comment", "proteomic profiling by mass spectrometry"),
        ]
        ...
        self.assertIn("organism[characteristics]", header_line)
        self.assertIn("NT=Orbitrap Fusion Lumos;AC=MS:1002732", header_line)
    ...
    def test_roundtrip_import_export(self):
        ...
        for i, header in enumerate(headers):
            ...
            column = MetadataColumnFactory.create_column(
                metadata_table=self.table, name=name_part, type=type_part, column_position=i, value=default_value
            )
            columns.append(column)
        ...
        # Reconstruct expected headers
        expected_headers = []
        for header in headers:
            if "[" in header and "]" in header:
                name_part = header.split("[")[0].strip()
                type_part = header.split("[")[1].rstrip("]").strip()
                expected_headers.append(f"{type_part}[{name_part}]")
            else:
                expected_headers.append(f"{header.strip()}[]")

        # Compare (order might be different due to processing)
        for expected in expected_headers:
            self.assertIn(expected, exported_header)
