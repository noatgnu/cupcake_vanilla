...
from .models import FavouriteMetadataOption, MetadataColumn, MetadataTable, SamplePool
...
class SDRFImportAPITest(APITestCase):
    ...
    def test_import_sdrf_with_real_fixture(self):
        ...
        self.assertTrue(response_data["pools_detected"])  # Should detect pools
        # This fixture doesn't have SN= rows, so sn_rows_count may be 0
        self.assertGreaterEqual(response_data["sn_rows_count"], 0)  # May or may not have SN= rows
        self.assertGreater(response_data["pooled_rows_count"], 0)  # Should have pooled rows
