...
class OntologyAPIValidationTest(APITestCase, QuickTestDataMixin):
    ...
    def test_species_search_api(self):
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            self.skipTest(f"Species API endpoint not available - status: {response.status_code}, url: {url}")
    ...
    def test_tissue_search_api(self):
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            self.skipTest(f"Tissue API endpoint not available - status: {response.status_code}, url: {url}")
    ...
    def test_ontology_suggestion_api(self):
        ...
        response = self.client.get(url, {"search": "homo"})

        if response.status_code == status.HTTP_200_OK:
            data = response.json()
            suggestions = data.get("results", [])

            # Should return species suggestions
            self.assertIsInstance(suggestions, list)
            if suggestions:
                suggestion = suggestions[0]
                self.assertIn("official_name", suggestion)
                # Check that homo sapiens might be found
                human_found = any("homo" in s.get("official_name", "").lower() for s in suggestions)
                if human_found:
                    self.assertTrue(human_found)
        else:
            self.skipTest(f"Species search API endpoint not available - status: {response.status_code}, url: {url}")
    ...
    def test_ms_terms_validation_api(self):
        ...
        OntologyFactory.create_ms_term(accession="MS:1001911", name="Q Exactive", term_type="instrument")

        # Use basic list endpoint with search parameter
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            self.skipTest(f"MS terms API endpoint not available - status: {response.status_code}, url: {url}")
    ...
    def test_unimod_search_api(self):
        ...
        OntologyFactory.create_unimod(
            accession="UNIMOD:4", name="Carbamidomethyl", definition="Carbamidomethylation of cysteine"
        )
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            self.skipTest(f"Unimod API endpoint not available - status: {response.status_code}, url: {url}")
...
class OntologyIntegrationValidationTest(TestCase, QuickTestDataMixin):
    ...
    def test_ontology_metadata_enrichment(self):
        ...
        ms_column = MetadataColumnFactory.create_column(
            metadata_table=self.table, name="comment", type="instrument", value="NT=6110 Quadrupole LC/MS;AC=MS:1000468"
        )
